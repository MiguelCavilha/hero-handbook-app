    ...
    <div className="print-sheet">
      ...
      <div className="print-page">
        ...
        <div className="print-header">
          ...
          <div className="print-identity">
            ...
            <div className="print-identity-grid">
              <PrintField label={t.race} value={`${character.race}${character.subrace ? ` (${character.subrace})` : ''}`} />
              <PrintField label={t.classLabel} value={character.classes.map(c => `${c.name} ${c.level}`).join(' / ')} />
              <PrintField label={t.background} value={character.background} />
              <PrintField label={t.alignment} value={character.alignment} />
              <PrintField label={t.level} value={String(level)} />
              <PrintField label={t.proficiency} value={formatModifier(prof)} />
              <PrintField label={t.experience} value={String(character.experience)} />
        ...
        <div className="print-combat-row">
          {[
            { label: t.armorClass, value: String(character.armorClass) },
            { label: t.initiative, value: formatModifier(character.initiative) },
            { label: t.speed, value: `${character.speed} ft` },
            { label: 'HP', value: `${character.hpCurrent}/${character.hpMax}` },
        ...
        {/* Two-column: abilities + saves | skills */}
        <div className="print-two-col">
          <div>
            ...
            {/* Saving throws */}
            <h3 className="print-section-title" style={{ marginTop: '0.75rem' }}>{t.savingThrows}</h3>
              ...
              {ABILITY_NAMES.map(ab => {
                ...
                return (
                  <div key={ab} className="print-save-row">
                    <span className={`print-dot ${prof ? 'filled' : ''}`} />
            ...
            {character.languages.length > 0 && (
              <>
                <h3 className="print-section-title" style={{ marginTop: '0.75rem' }}>{t.languages}</h3>
                <p className="print-tags">{character.languages.join(', ')}</p>
              </>
            )}
          </div>
              ...
              {character.skills.map(skill => {
                ...
                return (
                  <div key={skill.name} className="print-skill-row">
                    ...
                    {skill.expertise && <span className="print-dot filled" style={{ marginLeft: '-6px' }} />}
                    <span className="print-skill-name">{SKILL_LABELS[skill.name]} <span className="print-skill-ab">({ab})</span></span>
        ...
        {character.conditions.length > 0 && (
          <>
            ...
            <p className="print-tags">{character.conditions.join(', ')}</p>
          </>
        )}
      </div>
      ...
      <div className="print-page">
          ...
          <>
            ...
            {character.features.map(f => (
              <div key={f.id} className="print-feature">
                <div className="print-feature-name">{f.name} <span className="print-feature-source">({f.source})</span>
                  {f.usesMax > 0 && <span className="print-feature-uses"> · {f.usesCurrent}/{f.usesMax}</span>}
                </div>
        ...
        {/* Resistances etc */}
        {(character.resistances.length > 0 || character.immunities.length > 0 || character.vulnerabilities.length > 0) && (
          <div className="print-two-col">
            {character.resistances.length > 0 && <div><h3 className="print-section-title">{t.resistances}</h3><p className="print-tags">{character.resistances.join(', ')}</p></div>}
            {character.immunities.length > 0 && <div><h3 className="print-section-title">{t.immunities}</h3><p className="print-tags">{character.immunities.join(', ')}</p></div>}
            {character.vulnerabilities.length > 0 && <div><h3 className="print-section-title">{t.vulnerabilities}</h3><p className="print-tags">{character.vulnerabilities.join(', ')}</p></div>}
          </div>
        )}
      </div>

      {/* ── PAGE 3: Profile + Notes ── */}
      <div className="print-page">
        <div className="print-two-col">
          <div>
            ...
            {character.appearanceNotes && <p className="print-block-text" style={{ marginTop: '0.5rem' }}>{character.appearanceNotes}</p>}
          </div>
          <div>
            {character.personalityTraits && <PrintBlock title={t.personalityTraits} text={character.personalityTraits} />}
...
function PrintBlock({ title, text }: { title: string; text: string }) {
  return (
